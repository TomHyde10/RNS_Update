...
function computeDueInfo(reports, now = new Date()) {
  ...
  for (const [lei, entry] of byLei) {
    result[lei] = {
      halfYear: {
        lastFiledAt: entry.halfYear ? entry.halfYear.toISOString() : null,
        status: classify(entry.halfYear, now, HALF_YEAR.periodDays, HALF_YEAR.graceDays),
        dueDate: estimatedDueDate(entry.halfYear, HALF_YEAR.periodDays, HALF_YEAR.graceDays),
      },
      annual: {
        lastFiledAt: entry.annual ? entry.annual.toISOString() : null,
        status: classify(entry.annual, now, ANNUAL.periodDays, ANNUAL.graceDays),
        dueDate: estimatedDueDate(entry.annual, ANNUAL.periodDays, ANNUAL.graceDays),
      },
    };
  }
  return result;
...
// The same deadline classify() compares `now` against, exposed as an actual
// calendar date (ISO string) rather than just a status bucket - so a caller
// like the filing calendar (app.js) can place this on a specific day. `null`
// when there's no prior filing to estimate a cycle from (status 'unknown').
function estimatedDueDate(lastFiledAt, periodDays, graceDays) {
  return lastFiledAt ? new Date(lastFiledAt.getTime() + (periodDays + graceDays) * DAY_MS).toISOString() : null;
}
