...
//
// `keyword` is deliberately never part of the encrypted payload (see
// sealView/normalizeView above) - it's a transient search term typed into
// a box, not data worth hiding the way a watchlist is, so it stays a
// plain, always-readable param regardless of whether `v` is present.
function resolveReportQuery(get) {
  const keyword = get('keyword') || '';
  const token = get('v');
  if (token == null || token === '') {
    return { query: { leis: get('leis'), days: get('days'), categories: get('categories'), keyword } };
  }
  const { view, status, error } = openView(token);
  return error ? { status, error } : { query: { ...view, keyword } };
}
