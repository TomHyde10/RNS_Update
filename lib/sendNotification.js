...
async function sendNotification(report) {
  ...
  // links, which are public with no auth needed. Skipped entirely when a
  // report has no url (e.g. an item missing a document link).
  const attachments = report.url ? [{ path: report.url, filename: filenameFromUrl(report.url) }] : undefined;
