...
// A cheap existence check for callers (server.js's scheduleNextDigestCheck())
// that only need to know whether *any* push subscription exists, to decide
// the scheduler's poll cadence - avoids pulling every row's full JSONB
// columns (leis/categories/keyword) just to compute a boolean.
async function hasSubscriptions() {
  await ensureSchema();
  const { rows } = await getPool().query('SELECT 1 FROM push_subscriptions LIMIT 1');
...
module.exports = { enabled, listPushSubscriptions, hasSubscriptions, upsertPushSubscription, deletePushSubscription, markPushChecked };
