...
function ensureSchema() {
  if (!schemaReady) {
    ...
    `));
    // A subscription-wide keyword (unlike prefs, which are per-company/
    // per-category) - matched the same way the main report list's own
    // Keyword field is (lib/fetchReports.js's matchesKeyword), so a filing
    // mentioning it surfaces in this digest even for a company/category
    // combination the subscription hasn't otherwise selected. Empty string
    // (the default) means no keyword matching, same as today.
    schemaReady = schemaReady.then(() => getPool().query(`
      ALTER TABLE notification_subscriptions
...
async function createSubscription({ email, scheduleType, sendTimeUtc, prefs, keyword }) {
  await ensureSchema();
  const id = crypto.randomUUID();
  const { rows } = await getPool().query(
    `INSERT INTO notification_subscriptions (id, email, schedule_type, send_time_utc, prefs, keyword)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`,
    [id, email, scheduleType || 'daily', sendTimeUtc || '08:00', JSON.stringify(prefs || {}), keyword || '']
  );
...
// remove-and-re-add.
async function updateSubscription(id, { scheduleType, sendTimeUtc, prefs, keyword }) {
  await ensureSchema();
  const { rows } = await getPool().query(
    `UPDATE notification_subscriptions
     SET schedule_type = $2, send_time_utc = $3, prefs = $4, keyword = $5
     WHERE id = $1
     RETURNING *`,
    [id, scheduleType || 'daily', sendTimeUtc || '08:00', JSON.stringify(prefs || {}), keyword || '']
  );
