...
// Trimmed/lowercased once per request, not per item. Exported so callers
// that build their own keyword-matching logic against `normalise()`'s
// output (e.g. lib/sendDigest.js's filterDigestReports) apply it the same
// way this module does, instead of re-deriving it.
function normaliseKeyword(keyword) {
  return typeof keyword === 'string' ? keyword.trim().toLowerCase() : '';
}

// Shared by lib/sendDigest.js and lib/sendPush.js, which both need to turn
// a "since" cutoff into a `days` window wide enough for this module's own
// fetchReports() to search, capped the same way a request/`days` window
// always is.
function daysSinceAsWindow(since, now) {
  return Math.min(MAX_WINDOW_DAYS, Math.max(1, Math.ceil((now.getTime() - since.getTime()) / (24 * 60 * 60 * 1000)) + 1));
}

// Also shared by lib/sendDigest.js and lib/sendPush.js - both need to know
// whether one already-fetched report was published strictly after a given
// cutoff, tolerating a missing/unparseable publishedAt the same way.
function isPublishedAfter(report, since) {
  const published = report.publishedAt ? new Date(report.publishedAt) : null;
  return Boolean(published) && !Number.isNaN(published.getTime()) && published > since;
}
