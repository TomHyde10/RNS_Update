...
// Real-world testing showed the `isins` query filter does NOT restrict
// results server-side - a request for one ISIN came back with the general,
// unfiltered market-wide RNS feed (every issuer, every disclosure type).
// That means a single page can't be trusted to contain the item we're
// after: the target disclosure can be dozens of pages further back once
// every OTHER company's RNS activity for the window is mixed in. So this
// walks pageCursor forward, accumulating items across pages, and relies
// entirely on isinOf()/isHalfYearOrAnnualReport() below to do the real
// filtering - the isins param above is sent anyway (harmless, and helps if
// a future Ticker deploy actually honours it) but is never trusted alone.
//
// This is bounded by MAX_PAGES to protect the weekly request quota: each
// user-triggered refresh can cost up to MAX_PAGES requests. Override via
// the TICKER_MAX_PAGES env var if your plan's quota allows more (or less).
const MAX_PAGES = parseInt(process.env.TICKER_MAX_PAGES, 10) || 10;
const PAGE_SIZE = 200;
...
// Walks pageCursor across the general RNS feed from `from` to now,
// accumulating every item, until one of:
//  - Ticker stops offering a nextCursor
//  - a page comes back empty
//  - the oldest item on a page already predates `from` (items are returned
//    newest-first, so nothing older is left to find - also a safety net in
//    case dateFrom itself isn't applied server-side either)
//  - MAX_PAGES is reached (quota guard)
async function fetchAllItems({ from, apiKey }) {
  ...
  for (; pagesFetched < MAX_PAGES; pagesFetched++) {
    const params = new URLSearchParams({ pageSize: String(PAGE_SIZE), dateFrom: from });
    ...
    try {
      ...
      return { items, pagesFetched, error: { status: 502, body: { error: 'Failed to reach Ticker API', details: String(err) } } };
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        return {
          ...
          error: {
            status: 429,
            body: { error: 'Ticker API rate limit hit (per-second throttle or weekly quota).', details: details.slice(0, 2000) },
          },
        };
      }
      return {
        items,
        pagesFetched,
        error: { status: response.status, body: { error: `Ticker API returned ${response.status}`, details: details.slice(0, 2000) } },
      };
    }
    ...
    const nextCursor = data.meta && data.meta.paging && data.meta.paging.nextCursor;
    const oldest = pageItems[pageItems.length - 1];
    const oldestTime = oldest && oldest.timestamp ? new Date(oldest.timestamp).getTime() : null;

    if (!nextCursor || pageItems.length === 0 || (oldestTime !== null && oldestTime < cutoff)) {
      pagesFetched += 1;
      break;
    }
    cursor = nextCursor;
...
async function fetchReports({ isins, dateFrom } = {}) {
  ...
  if (effectiveIsins.length === 0) {
    return { status: 200, body: { isins: [], scanned: 0, pagesFetched: 0, count: 0, reports: [] } };
  }

  const from = dateFrom || defaultDateFrom();

  const { items, pagesFetched, error } = await fetchAllItems({ from, apiKey });
  if (error) return error;

  // The real filter: Ticker's own isins param can't be trusted (see
  // fetchAllItems' comment), so every item accumulated across all fetched
  // pages is checked against the requested ISINs here.
  const isinSet = new Set(effectiveIsins);
  const isWatched = (item) => isinSet.has(isinOf(item));
  const reports = items.filter(isWatched).filter(isHalfYearOrAnnualReport).map(normalise);

  // Diagnostic view limited to items matching a requested ISIN (matched
  // report-type or not) - shows why an expected disclosure is missing
  // without dumping every unrelated issuer's items from a multi-page scan.
  const scannedItems = items.filter(isWatched).map((item) => ({
    isin: isinOf(item),
